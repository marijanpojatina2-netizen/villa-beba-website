  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col justify-end px-6 pb-12 lg:px-10 lg:pb-16">
        ...
        <div className="mt-6 max-w-[280px] text-left lg:absolute lg:right-10 lg:bottom-16 lg:mt-0 lg:text-right"><p className="font-accent text-sm italic leading-relaxed text-white/80 lg:text-base">{t('season2026')}</p><p className="mt-4 text-[0.75rem] leading-relaxed text-white/50 font-body">{t('transparentPricing')}</p></div>
      </div>
    </section>
  );
}
...
function WhatsIncluded() {
  ...
  useEffect(() => { if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return; const ctx = gsap.context(() => { if (!sectionRef.current) return; const cards = sectionRef.current.querySelectorAll('.inc-card'); cards.forEach((card, i) => { gsap.fromTo(card, { y: 40, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, delay: i * 0.1, scrollTrigger: { trigger: card, start: 'top 85%' } }); }); }); return () => ctx.revert(); }, []);
  return (
