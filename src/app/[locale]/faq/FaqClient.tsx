    ...
    <main>
      <section className="relative h-[50vh] min-h-[400px] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 pb-12 lg:px-10 lg:pb-16">
          <div ref={titleRef} className="max-w-[90vw]"><div className="overflow-hidden"><h1 className="hero-line display-hero text-white">FAQ</h1></div></div>
      ...
      <section className="section-editorial bg-bg"><div className="mx-auto max-w-[900px] px-6 lg:px-10"><p className="label-section mb-16">(QUESTIONS)</p><div ref={accordionRef} className="divide-y divide-line">
        {faqData.map((item, index) => (
          <div key={index} id={index === cancellationIndex ? 'cancellation' : undefined} className="faq-item group scroll-mt-24">
            ...
            <div className={`overflow-hidden transition-all duration-500 ${openIndex === index ? 'max-h-96 pb-8' : 'max-h-0'}`}><p className="text-sm leading-relaxed text-text-muted md:text-base">{item.a}</p></div>
          </div>
        ))}
      </div></div></section>
