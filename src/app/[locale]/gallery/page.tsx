...
export default function GalleryPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const items = gridRef.current!.querySelectorAll('.gallery-thumb');
      gsap.fromTo(items, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, stagger: 0.03, ease: 'power2.out' });
    });
    return () => ctx.revert();
  }, [activeFilter]);
  ...
  return (
    <main>
      ...
      <section className="relative h-[50vh] min-h-[400px] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 pb-12 lg:px-10 lg:pb-16">
          <div ref={titleRef} className="max-w-[90vw]">
            <div className="overflow-hidden"><h1 className="hero-line display-hero text-white">GALLERY</h1></div>
          </div>
      ...
      {/* ──────── Filter Bar ──────── */}
      <section className="sticky top-0 z-30 border-b border-line bg-bg/95 backdrop-blur-md">
          ...
          <div className="flex gap-2 overflow-x-auto py-4 scrollbar-hide">
            {filters.map((filter) => (
              <button
                ...
                className={`btn-editorial shrink-0 !text-[0.625rem] !px-5 !py-2 ${activeFilter === filter.key ? '!bg-text !text-bg !border-text' : ''}`}
              >
                {filter.label}
              </button>
            ))}
          ...
          <div ref={gridRef} className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {filtered.map((item, index) => (
              <button
                ...
                className="gallery-thumb group relative aspect-[4/3] overflow-hidden rounded-sm border border-line transition-all duration-500 hover:border-line-strong"
              >
                <Image
                  src={item.src}
                  alt={item.alt}
      ...
      {lightboxIndex !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-bg/95 backdrop-blur-md" onClick={closeLightbox}>
          <button onClick={closeLightbox} className="absolute right-6 top-6 z-60 text-text-muted transition-colors hover:text-text" aria-label={tCommon('close')}>
            <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
          <button onClick={(e) => { e.stopPropagation(); goPrev(); }} className="absolute left-4 top-1/2 z-60 -translate-y-1/2 rounded-full border border-line bg-bg/60 p-3 text-text-muted transition-colors hover:border-line-strong hover:text-text md:left-8" aria-label={tCommon('previous')}>
            <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <div className="relative mx-16 aspect-[4/3] w-full max-w-4xl overflow-hidden rounded-sm border border-line" onClick={(e) => e.stopPropagation()}>
          ...
          <button onClick={(e) => { e.stopPropagation(); goNext(); }} className="absolute right-4 top-1/2 z-60 -translate-y-1/2 rounded-full border border-line bg-bg/60 p-3 text-text-muted transition-colors hover:border-line-strong hover:text-text md:right-8" aria-label={tCommon('next')}>
            <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
            </svg>
          </button>
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-sm tracking-wider text-text-dim">
