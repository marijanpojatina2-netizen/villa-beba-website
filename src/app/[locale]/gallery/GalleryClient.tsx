...
export default function GalleryPage() {
  ...
  return (
    <main>
      ...
      <section className="section-editorial bg-bg"><div className="mx-auto max-w-[1400px] px-6 lg:px-10"><div ref={gridRef} className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">{filtered.map((item, index) => (<button key={`${item.src}-${index}`} onClick={() => openLightbox(index)} className="gallery-thumb group relative aspect-[4/3] overflow-hidden rounded-sm border border-line transition-all duration-500 hover:border-line-strong"><Image src={item.src} alt={item.alt} fill className="object-cover transition-transform duration-700 group-hover:scale-110" loading={index < 8 ? 'eager' : 'lazy'} quality={60} sizes="(max-width: 768px) 50vw, (max-width: 1024px) 33vw, 25vw" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-bg/80 to-transparent p-4"><p className="text-left text-xs font-medium uppercase tracking-wider text-text-muted">{'villa' in item ? (item as { villa: string }).villa === 'ballena' ? 'Ballena' : (item as { villa: string }).villa === 'beluga' ? 'Beluga' : 'Complex' : ''}</p></div><div className="absolute inset-0 flex items-center justify-center bg-bg/60 opacity-0 transition-opacity duration-300 group-hover:opacity-100"><span className="btn-editorial !text-[0.6rem]">{t('view')}</span></div></button>))}</div></div></section>
