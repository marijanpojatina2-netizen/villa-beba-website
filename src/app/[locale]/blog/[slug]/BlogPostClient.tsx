    ...
    <main>
      <section className="relative flex h-[50vh] min-h-[400px] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-[900px] px-6 lg:px-10 pb-12 lg:pb-16">
          ...
          <time className="mt-4 block font-heading text-[0.6875rem] tracking-wider text-text-dim">{new Date(post.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</time>
        </div>
      </section>

      <section className="section-editorial bg-bg"><div className="mx-auto max-w-[700px] px-6 lg:px-10">
        <article>
          <p className="text-lg leading-relaxed text-text-muted">The rolling hills of Istria, dotted with centuries-old olive groves and vineyards, have long been a sanctuary for those who seek an authentic Mediterranean experience. Nestled in the heart of this peninsula, the medieval village of Svetvincenat offers a gateway to a world where time seems to slow, where every meal tells a story, and where the landscape itself becomes a companion to your journey.</p>
          <div className="my-10 line-h" />
          <p className="text-base leading-relaxed text-text-muted">Istria has been called many things: the new Tuscany, Croatia&apos;s hidden gem, a food lover&apos;s paradise. Yet none of these labels quite capture its essence. It is a place of contrasts, where the azure waters of the Adriatic meet ancient hilltop towns shrouded in morning mist, where Roman ruins stand beside contemporary art galleries, and where a simple plate of hand-rolled pasta becomes an unforgettable experience.</p>
          <p className="mt-6 text-base leading-relaxed text-text-muted">For visitors staying at Villa Ballena or Villa Beluga, the experience begins the moment you arrive. The warm stone, the scent of Mediterranean herbs drifting through open windows, the sound of cicadas on a summer evening — these are the details that transform a holiday into a memory. The villas themselves are designed to be a seamless extension of the Istrian landscape, with materials and aesthetics that honor the region&apos;s traditions while embracing contemporary luxury.</p>
          <p className="mt-6 text-base leading-relaxed text-text-muted">Beyond the villas, the region unfolds like a series of beautifully curated chapters. Drive fifteen minutes in any direction and you might find yourself at a family winery pouring biodynamic Malvazija, at a coastal town where the day&apos;s catch is grilled over open flames, or at a forest clearing where trained dogs sniff out the prized white truffle. Each experience is distinct yet connected by the common thread of Istrian hospitality — warm, genuine, and unhurried.</p>
          <p className="mt-6 text-base leading-relaxed text-text-muted">Whether you come for the gastronomy, the beaches, the culture, or simply the peace of a private pool under the Croatian sun, Istria rewards curiosity. It is a destination that reveals itself slowly, layer by layer, inviting you to return again and again. And with Villa Ballena and Villa Beluga as your home base, every adventure begins and ends in comfort.</p>
        </article>
        ...
        <div className="mt-16 rounded-sm border border-line bg-bg-elevated p-8 text-center md:p-12">
          ...
          <p className="mx-auto mt-4 max-w-md text-sm leading-relaxed text-text-muted">Make Istria your home for a week. Explore the region from the comfort of a luxury villa with a heated pool, private sauna, and everything you need.</p>
          <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row"><Link href="/villa-ballena" className="btn-editorial">Explore Villa Ballena</Link><Link href="/contact" className="btn-editorial">Check Availability</Link></div>
        </div>

        <div className="mt-12"><Link href="/blog" className="inline-flex items-center gap-2 font-heading text-[0.6875rem] font-medium uppercase tracking-[0.15em] text-text transition-all duration-300 hover:gap-3"><svg className="h-4 w-4 rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>Back to Blog</Link></div>
      </div></section>
