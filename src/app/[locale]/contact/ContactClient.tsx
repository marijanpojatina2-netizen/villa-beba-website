      ...
      <section className="section-editorial bg-bg"><div className="mx-auto max-w-[1400px] px-6 lg:px-10"><div className="grid gap-12 lg:grid-cols-5 lg:gap-16">
        <div className="lg:col-span-3"><p className="label-section mb-12">(INQUIRY)</p>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid gap-6 sm:grid-cols-2"><div><label htmlFor="name" className={labelClasses}>{t('name')} *</label><input id="name" type="text" className={inputClasses} placeholder="John Smith" {...register('name', { required: true })} />{errors.name && <p className="mt-1 text-xs text-red-400">Required</p>}</div><div><label htmlFor="email" className={labelClasses}>{t('email')} *</label><input id="email" type="email" className={inputClasses} placeholder="john@example.com" {...register('email', { required: true, pattern: /^\S+@\S+$/i })} />{errors.email && <p className="mt-1 text-xs text-red-400">Valid email required</p>}</div></div>
            ...
            <div><label htmlFor="message" className={labelClasses}>{t('message')}</label><textarea id="message" rows={5} className={inputClasses} placeholder="Tell us about your trip..." {...register('message')} /></div>
            <div><label htmlFor="howFound" className={labelClasses}>{t('howFound')}</label><select id="howFound" className={inputClasses} {...register('howFound')}><option value="">Select...</option><option value="google">Google</option><option value="instagram">Instagram</option><option value="airbnb">Airbnb</option><option value="booking">Booking.com</option><option value="friend">Friend / Recommendation</option><option value="other">Other</option></select></div>
