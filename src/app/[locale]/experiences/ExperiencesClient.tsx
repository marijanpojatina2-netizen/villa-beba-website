...
const categoryLabels: Record<Category, string> = {
  all: 'All', gastronomy: 'Gastronomy', daytrip: 'Day Trips', nature: 'Nature', sports: 'Sports', family: 'Family', culture: 'Culture',
};
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col justify-end px-6 pb-12 lg:px-10 lg:pb-16">
        <div ref={titleRef} className="max-w-[90vw]"><div className="overflow-hidden"><h1 className="hero-line display-hero text-white">ISTRIAN</h1></div><div className="overflow-hidden"><h1 className="hero-line display-hero text-white italic">Experiences</h1></div></div>
        <div className="absolute right-6 bottom-12 lg:right-10 lg:bottom-16 max-w-[280px] text-right"><p className="font-accent text-sm italic leading-relaxed text-white/80 lg:text-base">Beyond the villa</p><p className="mt-4 text-[0.75rem] leading-relaxed text-white/50 font-body">Your villa is the beginning. Istria is the adventure.</p></div>
      </div>
    </section>
  );
}
...
function ExperienceCards() {
  ...
  return (
    <section ref={sectionRef} className="section-editorial bg-bg"><div className="mx-auto max-w-[1400px] px-6 lg:px-10"><p className="label-section mb-12">(EXPLORE)</p><div className="mb-12 flex flex-wrap gap-2">{categories.map((cat) => (<button key={cat} onClick={() => setActiveCategory(cat)} className={`btn-editorial !text-[0.625rem] !px-5 !py-2 ${activeCategory === cat ? '!bg-text !text-bg !border-text' : ''}`}>{categoryLabels[cat]}</button>))}</div><div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{filteredExperiences.map((exp, i) => (<div key={exp.title} className="exp-card group overflow-hidden rounded-sm border border-line bg-bg-elevated transition-all duration-500 hover:border-line-strong"><div className="aspect-[16/10] relative overflow-hidden"><Image src={getExperienceImage(exp.title, i)} alt={exp.title} fill className="object-cover transition-transform duration-700 group-hover:scale-110" quality={75} /><div className="absolute inset-0 bg-gradient-to-t from-bg-elevated/90 via-transparent to-transparent" /><div className="absolute right-4 top-4"><span className="rounded-full border border-line bg-bg/80 px-3 py-1 text-xs tracking-wide text-text-muted backdrop-blur-sm">{exp.distance}</span></div><div className="absolute left-4 top-4"><span className="rounded-full border border-line bg-bg/40 px-3 py-1 text-[10px] uppercase tracking-wider text-text-muted backdrop-blur-sm">{categoryLabels[exp.category as Category] || exp.category}</span></div></div><div className="p-6"><h3 className="font-heading text-sm font-medium uppercase tracking-[0.12em] text-text">{exp.title}</h3><p className="mt-3 text-sm leading-relaxed text-text-muted">{exp.description}</p><p className="mt-4 micro-italic">We can arrange this for you</p></div></div>))}</div></div></section>
  );
}

function ExperiencesCTA() {
  return (<section className="bg-bg"><div className="line-h" /><div className="py-32 lg:py-48"><div className="mx-auto max-w-[900px] px-6 lg:px-10 text-center"><h2 className="display-lg">Let Us Curate<br />Your Istrian Adventure</h2><p className="mt-6 micro-italic">Tell us your interests and we will create your perfect itinerary</p><Link href="/contact" className="btn-editorial mt-12">Plan My Experiences</Link></div></div><div className="line-h" /></section>);
}
